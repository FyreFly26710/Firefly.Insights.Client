...
export const apiAuthLogin = async (request: UserLoginRequest) => {
    const response = await axiosClient.post<UserLoginResponse>('/identity/auth/login', request);
    return response.data as UserLoginResponse;
}
export const apiAuthRegister = async (request: UserRegisterRequest) => {
    const response = await axiosClient.post<boolean>('/identity/auth/register', request);
    return response.data as boolean;
}
export const apiAuthGetLoginUser = async () => {
    const response = await axiosClient.get<UserDto>('/identity/auth/getLoginUser');
