...
export const AiModelsGrid: React.FC<AiModelsGridProps> = ({ models, isLoading, error, onEdit, onDelete }) => {
    const columns: GridColDef<AiModelDto>[] = [
        ...
        },
        {
            ...
            width: 100,
            sortable: true,
            renderCell: (params) => <StatusChip value={params.value} trueLabel="Active" falseLabel="Inactive" />,
        },
        {
            field: 'avatar',
            headerName: 'Avatar',
        ...
        // },
        {
            ...
            align: 'right',
            headerAlign: 'right',
            renderCell: (params) => <TableActions id={params.row.aiModelId} onEdit={onEdit} onDelete={onDelete} />,
        },
    ];
    ...
    if (error) {
        return (
            <Paper sx={{ p: 3 }}>
                <Alert severity="error">Failed to load AI models: {error.message}</Alert>
            </Paper>
        );
    }
